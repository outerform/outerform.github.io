...
\newcommand{\CVPhoneFieldZH}{}
\ifdefined\CVPhone
  \renewcommand{\CVPhoneFieldZH}{& \textbf{电话:} \CVPhone}
\fi
...
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
  ...
  \textbf{地址:} 北京大学新燕园校区 & \textbf{电子邮件:} \href{mailto:liujunhao@pku.edu.cn}{liujunhao@pku.edu.cn} \\

  \textbf{主页:} https://outerform.site \CVPhoneFieldZH \\
\end{tabular*}

\leftskip=1.5em
\section{个人简介}
我的研究聚焦可解释人工智能（XAI），尤其关注面向 LLM 与多模态模型的可扩展解释、优化与控制方法。除学术研究外，我在腾讯混元参与过多模态/图像生成模型研发，在小红书 Dots 团队参与大语言模型预训练数据建设，覆盖代码数据处理、数据混合配比配置与训练数据构建。长期算法竞赛训练（NOI 金牌、ICPC/CCPC 金牌）也为我提供了扎实的算法基础与工程实现能力。
...
\section{实习经历}
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{工作内容}
          {参与大语言模型预训练数据建设，负责代码数据处理、数据混合配比配置与训练数据构建。}
      \resumeItemListEnd
    \resumeSubheading
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Liu, Junhao}, Haonan Yu, Zhenyu Yan, and Xin Zhang. \href{https://arxiv.org/abs/2505.12509}{Revitalizing Black-Box Interpretability: Actionable Interpretability for LLMs via Proxy Models.} ACL 2026 主会（Oral, 4.12\%）。\vspace{0.2cm}\\
    {\small \textbf{摘要：}大语言模型的模型无关解释因高昂的扰动成本而难以实现。我们提出基于代理模型的解释框架，通过筛选-应用机制将低成本模型的解释迁移至目标LLM，以仅9.5\%的成本实现90\%以上的解释保真度，并在提示压缩和有毒样本移除等下游任务中验证了其有效性。}
...
\end{itemize}
\vspace{-8pt}
{\small \hfill \textsuperscript{*}相同贡献}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Liu, Junhao}, Jian-Wei Zhang, Tao Huang, Miles Yang, Zhao Zhong, and Liefeng Bo. \href{https://outerform.site/assets/pdf/papers/atlas.pdf}{Think, Plan, Paint: Layout-Aware Reasoning for Controllable Image Generation in Unified Models.}（2026）。\vspace{0.2cm}\\
        {\small \textbf{摘要：}统一多模态大语言模型在可控图像生成中仍难以遵循复杂空间指令与逻辑约束。我们提出 ATLAS，以布局作为贯穿空间推理、显式物体规划与图像渲染的共享表示，形成 ``Think, Plan, and Paint'' 范式。ATLAS 在图像生成基准上取得了统一多模态模型中的领先表现，并通过同一布局接口支持指令引导编辑和多模态定位。}

    \item \textbf{Liu, Junhao}, Haonan Yu, and Xin Zhang. \href{https://arxiv.org/pdf/2410.12439}{ConLUX: Concept-Based Local Unified Explanations.}（2024）。 \vspace{0.2cm}\\
